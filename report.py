...
def build_month_report(
    ...
    rows: [(item, amount, category, "dd/mm"), ...] — mới nhất trước (từ db)
    Trả về BytesIO — file nằm trong RAM, gửi thẳng qua Telegram không cần
    ghi ra đĩa (đỡ phải dọn file tạm).
    """
    wb = Workbook()

    # ── Sheet 1: từng khoản chi, cũ -> mới cho dễ đọc ────────────────────
    ws = wb.active
    ws.title = "Chi tiết"
    ws.append(["Ngày", "Khoản chi", "Nhóm", "Số tiền (đ)"])
    for cell in ws[1]:
        cell.font = HEADER_FONT

    for item, amount, category, day in reversed(rows):
        ws.append([day, item, category, amount])
    ...
    for col, width in zip("ABCD", (10, 30, 14, 14)):
        ws.column_dimensions[col].width = width

    # ── Sheet 2: tổng theo nhóm + dòng tổng cộng ─────────────────────────
    ws2 = wb.create_sheet("Tổng hợp")
    ws2.append([f"Chi tiêu tháng {year_month[5:7]}/{year_month[:4]}", ""])
    ...
    ws2.append(["TỔNG CỘNG", sum(total for _, total in summary)])
